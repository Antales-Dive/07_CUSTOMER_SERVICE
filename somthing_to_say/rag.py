...
def should_use_knowledge_base(content: str) -> bool:
    """Return whether a message asks for campus facts rather than emotional support."""
    normalized = content.strip()
    return bool(normalized) and any(keyword in normalized for keyword in _KNOWLEDGE_BASE_KEYWORDS)
...
def build_faq_search_tool(vectorstore: Chroma):
    ...
    - 过滤后若为空，说明知识库里没有这个问题，返回明确提示，
      引导 Agent 不要硬编答案，并说明用户可自行查看官方帮助渠道。
    """
    ...
    def _search(query: str) -> str:
        ...
        if not results:
            return (
                f"{no_answer_prefix}。请如实告知用户当前问题不在常见问题范围内，不要编造；"
                "用户可以自行打开页面上的“找真人聊聊”查看官方帮助渠道。"
            )
    ...
    return StructuredTool.from_function(
        ...
        description=(
            ...
            "说明知识库中没有该问题的答案，不要编造；应如实告知用户，并说明用户可以"
            "自行打开页面上的“找真人聊聊”查看官方帮助渠道，不要调用或承诺转接。"
        ),
    )
