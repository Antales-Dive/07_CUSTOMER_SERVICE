...
RUNTIME_GUIDANCE_START = "## Runtime Guidance"
_SUPPORT_SKILL_PATH = Path(__file__).resolve().parent / "skills" / "campus-emotional-support" / "SKILL.md"


@lru_cache(maxsize=1)
def load_runtime_support_guidance() -> str:
    """Load only the bounded runtime portion of the project support Skill."""
    try:
        skill = _SUPPORT_SKILL_PATH.read_text(encoding="utf-8")
    except OSError:
        return ""
    _, marker, remainder = skill.partition(RUNTIME_GUIDANCE_START)
    if not marker:
        return ""
    guidance, _, _ = remainder.partition("\n## ")
...
async def build_agent(
    *,
    ...
    """构建一个绑定指定会话上下文的 Agent。

    参数:
        session_id: 当前会话 ID（保留用于调用方兼容）
    ...
    runtime_guidance = load_runtime_support_guidance()
    system_prompt = f"{base_prompt}\n\n{runtime_guidance}" if runtime_guidance else base_prompt
